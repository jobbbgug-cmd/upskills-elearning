...
export default function TeacherCouponsPage() {
  ...
  useEffect(() => {
    const savedTab = sessionStorage.getItem("teacher-coupons-tab");
    const savedFilter = sessionStorage.getItem("teacher-coupons-filter");
    const savedSearch = sessionStorage.getItem("teacher-coupons-search");
  ...
  useEffect(() => {
    if (isMounted) {
      sessionStorage.setItem("teacher-coupons-tab", activeTab);
    }
  }, [activeTab, isMounted]);
  ...
  useEffect(() => {
    if (isMounted) {
      sessionStorage.setItem("teacher-coupons-filter", filterActive);
    }
  ...
  useEffect(() => {
    if (isMounted) {
      sessionStorage.setItem("teacher-coupons-search", search);
    }
  }, [search, isMounted]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/teacher-online/coupons?type=${activeTab}`);
      if (!res.ok) {
        setItems([]);
        setLoading(false);
        return;
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/teacher-online/coupons", {
        method: "POST",
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {item.itemType === "coupon" && item.type && item.value !== undefined && (
                        <span className="text-lg px-4 py-2 rounded-full font-semibold bg-indigo-100 text-indigo-700">
                          {item.type === "percent" ? `ลด ${item.value}%` : `ลด ฿${item.value.toLocaleString()}`}
                        </span>
                      )}
                      {item.itemType === "promotion" && item.type && item.value !== undefined && (
                        <span className="text-lg px-4 py-2 rounded-full font-semibold bg-purple-100 text-purple-700">
                          {item.type === "percent" ? `ลด ${item.value}%` : `ลด ฿${item.value.toLocaleString()}`}
                        </span>
                      )}
                      {item.itemType === "package" && item.price && (
                        <span className="text-lg px-4 py-2 rounded-full font-semibold bg-teal-100 text-teal-700">
