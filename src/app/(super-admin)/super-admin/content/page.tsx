...
export default function SuperAdminContentPage() {
  ...
  const load = async (institutionId?: string) => {
    ...
    const [contentRes, instRes] = await Promise.all([
      fetch(`/api/admin/content${qs}`),
  ...
  const filtered = contents.filter((c) => {
    ...
    return !q || c.name.toLowerCase().includes(q) || (c.description?.toLowerCase().includes(q) ?? false);
  });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">เนื้อหาทั้งหมดจากทุกสถาบัน</p>
        </div>
      ...
      <div className="flex gap-3 mb-5 flex-wrap items-center">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ค้นหาชื่อเนื้อหา..."
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">ชื่อเนื้อหา</th>
                <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">สถาบัน</th>
                <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">คำอธิบาย</th>
      ...
      <p className="text-xs text-gray-400 mt-3 text-right">แสดง {filtered.length} จาก {contents.length} รายการ</p>
    </div>
  );
}
