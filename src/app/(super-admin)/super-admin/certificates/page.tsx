...
import { Award, Search, Plus, Trash2, Building2, X, Check, AlertCircle } from "lucide-react";
...
export default function SuperAdminCertificatesPage() {
  ...
  const [createForm, setCreateForm] = useState({ studentId: "", courseId: "", title: "", description: "", institutionId: "" });
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [certsRes, instRes] = await Promise.all([
      ...
      if (instRes.ok) {
        ...
        insts.forEach((i: any) => { names[i._id] = i.name; });
  ...
  // Load students and courses when institution changes
  useEffect(() => {
    const loadStudentsAndCourses = async () => {
      try {
        const qs = filterInstitution ? `?institutionId=${filterInstitution}` : "";
  ...
  const filtered = certificates.filter((c) => {
    ...
    const matchInst = !filterInstitution || c.institutionId === filterInstitution;
    return matchSearch && matchStudent && matchCourse && matchInst;
  });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCreateForm({ studentId: "", courseId: "", title: "", description: "", institutionId: "" });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">จัดการใบรับรองของทั้งระบบ</p>
        </div>
        <button
      ...
      <div className="flex gap-3 mb-5 flex-wrap items-center">
        ...
        <select
          ...
          className="px-3 py-2.5 border rounded-xl text-sm bg-white focus:outline-none focus:ring-2"
                    ...
                    <div className="flex items-center gap-4 mt-3 text-sm text-gray-600 flex-wrap">
                      {cert.institutionId && (
                        <span className="inline-flex items-center gap-1">
                          ...
                          {institutionNames[cert.institutionId] || "สถาบัน"}
                        </span>
                      )}
