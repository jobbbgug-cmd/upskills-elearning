...
export default function SuperAdminProductsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === "product") {
        ...
        const res = await fetch(`/api/super-admin/courses?${params.toString()}`);
        if (!res.ok) {
          setCourses([]);
          setLoading(false);
          return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editing
        ? `/api/super-admin/${endpoint}/${editing._id}`
        : `/api/super-admin/${endpoint}`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const endpoint = activeTab === "product" ? "products" : "courses";
      const res = await fetch(`/api/super-admin/${endpoint}/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        if (activeTab === "product") {
