...
export default function SuperAdminOrdersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/super-admin/sales");
      if (!res.ok) {
        setOrders([]);
        setLoading(false);
        return;
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    setActing(id);
    try {
      const res = await fetch(`/api/super-admin/orders/${id}`, {
        method: "PATCH",
  ...
  const handleDelete = async (id: string) => {
    ...
    setActing(id);
    try {
      const res = await fetch(`/api/super-admin/orders/${id}`, { method: "DELETE" });
      if (res.ok) {
