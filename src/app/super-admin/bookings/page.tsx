  ...
  const load = async (institutionId?: string) => {
    ...
    try {
      const [bookingsRes, instRes] = await Promise.all([
        fetch(`/api/super-admin/bookings${qs}`),
  ...
  return (
    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full">
            <h2 className="text-lg font-bold text-gray-900 mb-2">
              {confirmBooking.action === "approve" ? "อนุมัติการชำระเงิน?" : "ปฏิเสธการชำระเงิน?"}
            </h2>
            <p className="text-sm text-gray-600 mb-6">
              {confirmBooking.action === "approve"
                ? "ยืนยันว่าสลิปถูกต้องและอนุมัติการจองนี้"
                : "การจองนี้จะถูกปฏิเสธ นักเรียนจะไม่ได้เข้าเรียน"}
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setConfirmBooking((d) => ({ ...d, open: false }))}
              ...
              <button
                onClick={() => {
                  act(confirmBooking.id, confirmBooking.action);
                  setConfirmBooking((d) => ({ ...d, open: false }));
                }}
                className={`flex-1 px-4 py-2 text-white rounded-lg font-medium ${
                  confirmBooking.action === "approve" ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"
                }`}
              >
                {confirmBooking.action === "approve" ? "อนุมัติ" : "ปฏิเสธ"}
              </button>
            </div>
          </div>
        </div>
          ...
          {filtered.map((booking) => {
            ...
            return (
              <div
                key={booking._id}
                className={cn(
                  "bg-white rounded-2xl border p-5 flex flex-col md:flex-row gap-4",
              ...
              >
                <div className="flex-1 space-y-2 min-w-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={cn("text-xs font-semibold px-2.5 py-1 rounded-lg", statusInfo.cls)}>
                      {statusInfo.label}
                    </span>
                  </div>
                ...
                <div className="flex flex-col items-center gap-1 shrink-0">
                  ...
                  {booking.slipImage ? (
                    <button
                      onClick={() => setPreviewSlip(booking.slipImage)}
                    ...
                    >
                      <img src={booking.slipImage} alt="slip" className="w-full h-full object-cover group-hover:opacity-80 transition-opacity" />
                      <span className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/20 text-white text-xs font-medium">
                        ดูสลิป
                      </span>
                    </button>
                  ) : (
                ...
                {isPending && (
                  <div className="flex md:flex-col gap-2 justify-end shrink-0">
                    <button
                      onClick={() => setConfirmBooking({ open: true, id: booking._id, action: "approve" })}
                    ...
                    <button
                      onClick={() => setConfirmBooking({ open: true, id: booking._id, action: "reject" })}
                      disabled={acting === booking._id}
      ...
      {previewSlip && (
        ...
        >
          <div
            className="bg-white rounded-2xl p-3 max-w-sm w-full shadow-2xl"
