...
export default function SuperAdminMembersPage() {
  ...
  const loadPending = useCallback(async () => {
    ...
    const [rPending, rAll] = await Promise.all([
      fetch("/api/super-admin/users/pending"),
  ...
  const doApprove = async (user: PendingUser) => {
    ...
    const res = await fetch(`/api/super-admin/users/${user._id}/approve`, { method: "POST" });
  ...
  const doReject = async (id: string) => {
    setProcessing(id);
    await fetch(`/api/super-admin/users/${id}/reject`, { method: "POST" });
